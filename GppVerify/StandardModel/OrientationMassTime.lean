...
theorem jacobianNumerator_eigenvalue_fourth_power
    (N4 : Matrix (Fin 4) (Fin 4) ℂ) (D lam : ℂ) (v : Fin 4 → ℂ)
    (hN4 : N4 * N4 * (N4 * N4) = D ^ 4 • (1 : Matrix (Fin 4) (Fin 4) ℂ))
    (hv : v ≠ 0)
    (heig : N4 *ᵥ v = lam • v) :
    lam ^ 4 = D ^ 4 := by
  have h2 : (N4 * N4) *ᵥ v = lam ^ 2 • v := by
    rw [← Matrix.mulVec_mulVec, heig, Matrix.mulVec_smul, heig, smul_smul, sq]
  have h4 : (N4 * N4 * (N4 * N4)) *ᵥ v = lam ^ 4 • v := by
    rw [← Matrix.mulVec_mulVec, h2, Matrix.mulVec_smul, h2, smul_smul]
    congr 1; ring
  rw [hN4] at h4
  have hscalar : D ^ 4 • v = lam ^ 4 • v := by
    simpa [Matrix.smul_mulVec, Matrix.one_mulVec] using h4
  by_contra hne
  have hcoeff : D ^ 4 - lam ^ 4 ≠ 0 := sub_ne_zero.mpr (Ne.symm hne)
  have hz : (D ^ 4 - lam ^ 4) • v = 0 := by
    ext i
