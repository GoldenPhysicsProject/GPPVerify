...
/-!
# Spectral closure for the Grassmannian order-four operator

This module closes the elementary complex-algebra step left after the proved
matrix identity `N^4 = D^4 I`: any nonzero eigenvector has eigenvalue among
the four fourth roots of `D^4`, hence the eigenvalue modulus is exactly `|D|`.
No global eigenvalue-multiset machinery is needed for this implication.
-/

namespace GppOrientationMassTime

/-- If two complex numbers have the same fourth power, they differ by a fourth
root of unity. -/
...
/-- Equal fourth powers imply equal complex norms. -/
...
/-- Eigenvector-level classification: a matrix satisfying `M^4 = D^4 I`
has no eigenvalues outside `{D,-D,iD,-iD}`. -/
...
/-- In the same hypotheses, every eigenvalue has modulus exactly `|D|`. -/
...
/-- Complexified universal tangent operator in row-major coordinates. -/
...
/-- The universal operator actually realizes the `+1` fourth-root mode. -/
...
/-- The universal operator actually realizes the `+i` fourth-root mode. -/
