...
theorem eigenvalue_fourth_root_classification
    (M : Matrix (Fin 4) (Fin 4) ℂ) (D lam : ℂ) (v : Fin 4 → ℂ)
    (hM4 : M * M * (M * M) = D ^ 4 • (1 : Matrix (Fin 4) (Fin 4) ℂ))
    (hv : v ≠ 0)
    (heig : M *ᵥ v = lam • v) :
    lam = D ∨ lam = -D ∨ lam = Complex.I * D ∨ lam = -Complex.I * D := by
  apply fourth_power_eq_fourth_power_cases
  exact jacobianNumerator_eigenvalue_fourth_power M D lam v hM4 hv heig
