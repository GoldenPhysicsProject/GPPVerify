...
theorem left_center_flip_reverses_vector (lambda lambdatilde : Spinor2) :
  ...
  have h1 : scale2 1 lambdatilde = lambdatilde := by
    rcases lambdatilde with ⟨x, y⟩; simp [scale2]
  have h := nullMomentum_biscaling (-1) 1 lambda lambdatilde
  rw [h1] at h
  simpa [centerScale] using h
...
theorem right_center_flip_reverses_vector (lambda lambdatilde : Spinor2) :
    nullMomentum lambda (centerScale (-1) lambdatilde) =
      scaleM2 (-1) (nullMomentum lambda lambdatilde) := by
  have h1 : scale2 1 lambda = lambda := by
    rcases lambda with ⟨x, y⟩; simp [scale2]
  have h := nullMomentum_biscaling 1 (-1) lambda lambdatilde
  rw [h1] at h
  simpa [centerScale] using h
