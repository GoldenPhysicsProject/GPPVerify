...
def ordinaryKernelSpinor (lam : Spinor2) : V4 := (0,0,lam.1,lam.2)

/-- Dual-twistor kernel embedding at flat infinity. -/
def dualKernelSpinor (lamt : Spinor2) : V4 := (lamt.1,lamt.2,0,0)

/-- Every ordinary kernel spinor is annihilated by the flat infinity Clifford map. -/
theorem ordinaryKernelSpinor_mem_kernel (lam : Spinor2) :
    cPlus infinityPoint (ordinaryKernelSpinor lam) = (0,0,0,0) := by
  rcases lam with ⟨l0,l1⟩
  simp [ordinaryKernelSpinor, cPlus, infinityPoint]

/-- Every dual kernel spinor is annihilated by the opposite map. -/
theorem dualKernelSpinor_mem_kernel (lamt : Spinor2) :
    cMinus infinityPoint (dualKernelSpinor lamt) = (0,0,0,0) := by
  rcases lamt with ⟨t0,t1⟩
...
def nullMomentum (lam lamt : Spinor2) : M2 :=
  (lam.1*lamt.1, lam.1*lamt.2, lam.2*lamt.1, lam.2*lamt.2)

/-- The spinor outer product is exactly null: its `2x2` determinant vanishes identically. -/
theorem nullMomentum_det_zero (lam lamt : Spinor2) :
    det2 (nullMomentum lam lamt) = 0 := by
  rcases lam with ⟨l0,l1⟩
  rcases lamt with ⟨t0,t1⟩
...
def scale2 (r : ℝ) (lam : Spinor2) : Spinor2 := (r*lam.1,r*lam.2)
...
theorem nullMomentum_biscaling (r s : ℝ) (lam lamt : Spinor2) :
    nullMomentum (scale2 r lam) (scale2 s lamt) =
      scaleM2 (r*s) (nullMomentum lam lamt) := by
  rcases lam with ⟨l0,l1⟩
  rcases lamt with ⟨t0,t1⟩
  apply Prod.ext
...
theorem nullMomentum_littleGroup_invariant
    (r : ℝ) (hr : r ≠ 0) (lam lamt : Spinor2) :
    nullMomentum (scale2 r lam) (scale2 (1/r) lamt) = nullMomentum lam lamt := by
  rw [nullMomentum_biscaling]
  have hs : r * (1/r) = 1 := by field_simp [hr]
  rw [hs]
  rcases nullMomentum lam lamt with ⟨a,b,c,d⟩
  simp [scaleM2]
