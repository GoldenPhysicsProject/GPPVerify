...
theorem splitComplement_involutive (A : M2) (hD : det2 A ≠ 0) :
  ...
  have hdet := det2_splitComplement A hD
  rcases A with ⟨a,b,c,d⟩
  simp only [det2] at hD
  set D := a * d - b * c with hDdef
  have hc : splitComplement (a, b, c, d) = (d / D, -c / D, -b / D, a / D) := by
    simp [splitComplement, det2, hDdef]
  have hdet' : det2 (d / D, -c / D, -b / D, a / D) = 1 / D := by rw [← hc]; exact hdet
  have e : ∀ x : ℝ, x / D / (1 / D) = x := fun x => by field_simp
  rw [hc]
  simp only [splitComplement]
  rw [hdet']
  simp only [neg_div, neg_neg, e]
...
theorem chartPlucker_splitComplement (A : M2) (hD : det2 A ≠ 0) :
  ...
  apply P6.ext <;> simp <;> field_simp <;> ring
...
theorem tau_eq_splitQuarterTurn_splitComplement (A : M2) :
  ...
  rcases A with ⟨a,b,c,d⟩
  simp [tau, splitQuarterTurn, splitComplement, det2, neg_div]
