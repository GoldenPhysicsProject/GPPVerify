...
/-- The Klein pairing of a vector with itself is twice its quadratic form. -/
theorem epsilonPair_self (q : P6) : epsilonPair q q = 2 * kleinQ q := by
  rcases q with ⟨q01,q02,q03,q12,q13,q23⟩
  simp [epsilonPair, kleinQ]; ring

/-- Linearity of the pairing in its first slot, along `p - t q`. -/
theorem epsilonPair_sub_scale (p q : P6) (t : ℝ) :
    epsilonPair (subP6 p (scaleP6 t q)) q = epsilonPair p q - t * epsilonPair q q := by
  rcases p with ⟨p01,p02,p03,p12,p13,p23⟩
  rcases q with ⟨q01,q02,q03,q12,q13,q23⟩
  simp [epsilonPair, subP6, scaleP6]; ring

/-- The quadratic form along `p - t q`. -/
theorem kleinQ_sub_scale (p q : P6) (t : ℝ) :
    kleinQ (subP6 p (scaleP6 t q)) = kleinQ p - t * epsilonPair p q + t ^ 2 * kleinQ q := by
  rcases p with ⟨p01,p02,p03,p12,p13,p23⟩
  rcases q with ⟨q01,q02,q03,q12,q13,q23⟩
  simp [kleinQ, epsilonPair, subP6, scaleP6]; ring

/-- Reflection sends its non-null normal to its negative. -/
theorem kleinReflection_normal
    (q : P6) (hq : kleinQ q ≠ 0) :
    kleinReflection q q = negP6 q := by
  have h2 : epsilonPair q q / kleinQ q = 2 := by rw [epsilonPair_self]; field_simp
  rw [kleinReflection, h2]
  rcases q with ⟨q01,q02,q03,q12,q13,q23⟩
  apply P6.ext <;> simp [subP6, scaleP6, negP6] <;> ring
...
theorem epsilonPair_reflection_normal
  ...
  rw [kleinReflection, epsilonPair_sub_scale, epsilonPair_self]
  field_simp
  ring
...
theorem kleinReflection_involutive
  ...
  have hdef : ∀ x, kleinReflection q x = subP6 x (scaleP6 (epsilonPair x q / kleinQ q) q) :=
    fun x => rfl
  rw [hdef (kleinReflection q p), hpair, hdef p]
  rcases p with ⟨p01,p02,p03,p12,p13,p23⟩
  rcases q with ⟨q01,q02,q03,q12,q13,q23⟩
  apply P6.ext <;> simp [subP6, scaleP6] <;> ring

/-- The reflection preserves the Klein quadratic form. -/
theorem kleinQ_kleinReflection
    (q p : P6) (hq : kleinQ q ≠ 0) :
    kleinQ (kleinReflection q p) = kleinQ p := by
  rw [kleinReflection, kleinQ_sub_scale]
  field_simp
  ring
